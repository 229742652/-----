...
(function ($) {
    $.fn.dragSwipe = function (option) {
        var _this = this;
        var _default = {
            direction: "horizontal", //"vertical" 滑动的方向，默认为水平方向
            port: document.documentElement.clientWidth,
            max: .3, //滑动的距离为可视区的多少部分时可以触发，
            fnComplate: function (e) {//运动完成后作的回调函数。
        ...
        setting.isOff && setting.offEvents();
        
        var len = this.width() / setting.port;
        if (setting.direction === "vertical") {
            len = this.height() / setting.port;
            this.css("-webkit-transition", "300ms -webkit-transform").css("-webkit-transform", "translate3d(0," + (-setting.iNow * setting.port) + "px,0)").off("webkitTransitionEnd");
        }
        else {
            this.css("-webkit-transition", "300ms -webkit-transform").css("-webkit-transform", "translate3d(" + (-setting.iNow * setting.port) + "px,0,0)").off("webkitTransitionEnd");
        }
        
        var transX = 0, transY = 0;
        if (setting.direction.toLowerCase() === "horizontal") {//左右滑动
            ...
            _this.on("touchstart", function (e) {
                ...
                var e = e.changedTouches[0];
                transX = e.pageX - _this.startX;

                _this.iNow++;
                if (_this.iNow === 1) {
                    if (Math.abs(e.pageY - _this.startY) - Math.abs(transX) > 0) {
                        _this.bStop = !_this.bStop;
                    }
                }
                if (!_this.bStop) {
                    ...
                    if (transX > 0) {
                        if (setting.iNow <= 0) {
                            _this.css("-webkit-transform", "translate3d(" + (transX - setting.iNow * setting.port) / 3 + "px,0,0)");
                        }
                        else {
                            _this.css("-webkit-transform", "translate3d(" + (transX - setting.iNow * setting.port) + "px,0,0)");
                        }
                    }
                    else {
                        if (setting.iNow >= len) {
                ...
                e.stopPropagation();
                var e = e.changedTouches[0];
                var disX = Math.abs(e.pageX - _this.startX);
                var dir = e.pageX - _this.startX > 0;

                var endTime = Date.now() * 1;
                if (dir) {

                    if (disX - setting.port * setting.max > 0 || (disX > 40) && (endTime - _this.startTime < 200)) {
                        setting.iNow--;
                        if (setting.iNow <= 0) {
                            setting.iNow = 0;
                        }
                ...
                _this.css("-webkit-transition", "300ms -webkit-transform").css("-webkit-transform", "translate3d(" + (-setting.iNow * setting.port) + "px,0,0)").off("webkitTransitionEnd");
                _this.on("webkitTransitionEnd", function () {
                    //iNow > 0 && (iNow <= len - 1) && setting.fnComplate(iNow);
                    setting.fnComplate(setting.iNow);
                })
                //Math.abs(disX - setting.port * setting.max);
            });


        }
        else if (setting.direction.toLowerCase() === "vertical") {//上下滑动
            ...
            _this.on("touchstart", function (e) {
                ...
                var e = e.changedTouches[0];
                transY = e.pageY - _this.startY;
                console.log(transY)
                if (transY > 0) {

                    if (setting.iNow <= 0) {
                        _this.css("-webkit-transform", "translate3d(0," + (transY - setting.iNow * setting.port) / 3 + "px,0)");
                    }
                    else {
                        _this.css("-webkit-transform", "translate3d(0," + (transY - setting.iNow * setting.port) + "px,0)");
                    }
                }
                else {
                    if (setting.iNow >= len) {
                ...
                e.stopPropagation();
                var e = e.changedTouches[0];
                var disY = Math.abs(e.pageY - _this.startY);
                var dir = e.pageY - _this.startY > 0;
                var endTime = Date.now() * 1;
                if (dir) {
                    if (disY - setting.port * setting.max > 0 || (disY > 40) && (endTime - _this.startTime < 200)) {
                        setting.iNow--;
                        if (setting.iNow <= 0) {
                            setting.iNow = 0;
                        }
                ...
                _this.css("-webkit-transition", "300ms -webkit-transform").css("-webkit-transform", "translate3d(0," + (-setting.iNow * setting.port) + "px,0)").off("webkitTransitionEnd");
                _this.on("webkitTransitionEnd", function () {
                    //iNow > 0 && (iNow <= len - 1) && setting.fnComplate(iNow);
                    setting.fnComplate(setting.iNow);
                });
