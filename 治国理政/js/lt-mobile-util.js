...
!function (w) {
    var Util = {
        loadImgByCanvas: function (aImgSrc, canvas, x, y, scaleX, scaleY, canvasW, canvasH) {
            var img = new Image();
            var context = canvas.getContext && canvas.getContext("2d");
            context && context.fillText("图片加载中,请稍等...", canvas.width / 2 - 40, canvas.height / 2);
            img.onload = function () {
                scaleX = scaleX || 1;
                scaleY = scaleY || 1;
        ...
        requestAnimFrame: (function () { //用来做动画的。
            ...
            var requestAnimationFrame, cancelAniFrame;
            for (var x = 0; x < vendors.length && !window.requestAnimationFrame; ++x) {
                requestAnimationFrame = window[vendors[x] + 'RequestAnimationFrame'];
                cancelAniFrame = window[vendors[x] + 'CancelAnimationFrame'] ||    // name has changed in Webkit
            ...
            if (!cancelAniFrame) {
                cancelAniFrame = function (id) {
                    clearTimeout(id);
                };
            }
            return {
                request: requestAnimationFrame,
                cancel: cancelAniFrame
            }
        })(),
        getMenuIcon: function (color) { //通过canvas生成移端的菜单图标。
            var canvas = document.createElement("canvas");
            var context = canvas.getContext && canvas.getContext("2d");
            context.lineWidth = 3;
            ...
            for (var i = 0; i < 3; i++) {
                context.beginPath()
                context.moveTo(5, (i + 1) * 8);
                context.lineTo(30, (i + 1) * 8);
                context.closePath();
                context.stroke();
            }
        ...
        ltLoading: function (arr, fn, option) {
            ...
            img1.onload = function () {
                img.onload = function () {
                    id = _this.requestAnimFrame.request(render);
                }
                img.src = src;
            };
            ...
            function render() {
                ...
                if (len > 0) {
                    ...
                    if (count / len >= 1) { //加载完成了。。
                        _this.requestAnimFrame.cancel(id);
                        fn && fn();
                    }
                    else {
                        id = _this.requestAnimFrame.request(render);
        ...
        ltLoading2: function (arr, fn, option) { //加载2
            ...
            var html = "#ltLoading{width: 430px;margin-top:50px;height: 20px;position: relative;border-radius:5px; border:1px solid " + color + ";padding: 1px;box-sizing:border-box;}\
                #ltLoading span{width: 100%;height:100%;background: "+ color + ";border-radius: 5px;display: inline-block;}\
                #ltLoading div{width:0;height: 100%;position: absolute;left: 0;top:0;z-index: 1;background: #fff;border-radius: 5px 0 0 5px;text-align: right;line-height: 25px;}\
                #ltLoading div label{position: relative;top:-3px;color:"+ color + "}\
                #ltLoading div img{position: absolute;top:-40px;}\
            ...
            var oDiv = _$("ltLoading").getElementsByTagName("div")[0];
        ...
        startMove: function (obj, json, times, fx, fn) { //运动框架
            ...
            obj.timer = window.requestAnimationFrame(move);
            function now() {
